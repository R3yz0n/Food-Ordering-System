...
const FoodItemCard = ({ item }) => {
    ...
    return (
        <motion.div className=' duration-200 bg-white drop-shadow-md hover:shadow-lg hover:shadow-gray-400  flex items-center w-[350px] xs:w-225  sm:w-300 md:w-[230px] lg:w-[290px] py-4 p-2 gap-1 rounded-lg mx-auto relative h-[10rem] food-item-card '>
            <LazyLoadImage className=' w-[8rem] xs:w-[6rem]  sm:max-w-[9rem] sm:min-w-[9rem]   md:min-w-[7rem] md:max-w-[7rem]   lg:min-w-[9rem] max-h-[9rem]   object-contain p-1 ' effect='opacity' src={`${APIURL}/file/${item.image}`} alt="FoodItem" delayTime={500} visibleByDefault={false} />
