...
import { getOrderById } from "../../store/order/orderAction";
...
const SingleOrderDetails = ({ selectedOrderId }) => {
  ...
  return (
    <section className=" w-[500px] bg-gray-200 rounded-md  pb-5  ">
      {/* No orders */}
      <h2 className="text-2xl font-semibold text-headingColor px-5  py-2 flex gap-4 w-fit bg-white rounded-tl-md  rounded-br-lg order-card  items-center z-20  ">
        <TbListDetails className="text-gray-200 text-2xl h-7 w-7 opacity-80 bg-red-600 rounded-sm p-[2px]  tracking-wider" />
        Order Details
      </h2>
      <aside className="bg-white mt-4 px-4 w-[96%] rounded-lg mx-auto text-textColor font-medium opacity-95 font-sans pt-3 pb-4 text-[15px] ">
        ...
        <h5 className="w-full flex gap-1 items-center my-1">
          ...
          <span
            className={`px-3  text-gray-100 py-[1px] -ml-1 text-[13px] rounded-lg tracking-wider ${getStatusBadgeClass(
        ...
        <h5 className="w-full flex gap-1">
          <span className="w-28">Sub Total Rs.</span>121212 /-
        </h5>

        {orderInfoById?.status === "Preparing" ? (
          <h4 className="text-[13px] mt-1 flex items-center gap-3 font-semibold">
            Do you want to cancel?{" "}
            <button className="bg-red-500 text-gray-100 px-3  rounded-lg py-[1px] text-[12px] hover:scale-95 duration-200 hover:bg-red-400">
              Yes
            </button>{" "}
          </h4>
        ) : (
      ...
      <aside className="grid grid-cols-2  mt-4 px-3 rounded-md gap-3 overflow-y-auto max-h-[345px] ">
